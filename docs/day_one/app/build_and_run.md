...
You've got a Dockerfile. Time to actually try it. Two commands turn that file into something running: `docker build` reads the Dockerfile and produces an *image* — the packaged blueprint from [What Is a Container, Really?](../what_is_a_container.md). `docker run` takes that image and starts a *container* from it: an actual running process. They're two separate steps, not one action with two names, and keeping them separate in your head is most of what this article is about.
